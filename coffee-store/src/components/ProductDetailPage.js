class ProductDetailPage {
  ...
  constructor($target, routeProps) {
    this.$target = $target;
    this.routeProps = routeProps;

    this.$productDetailPage = document.createElement('div');
    this.$productDetailPage.className = 'ProductDetailPage';
    $target.appendChild(this.$productDetailPage);

    this.$productDetailPage.addEventListener('click', this.handleOrderClick);
  ...
  handleOrderClick = (e) => {
    if (e.target.className === 'OrderButton') {
      this.routeProps.push('/web/cart');
    }
  };

  render() {
    this.$productDetailPage.innerHTML = `
      <h1>커피잔 상품 정보</h1>
